...
const MOCK_SUBSCRIBERS = [
  ...
  },
  {
    ...
    status: 'unsubscribed',
    lists: ['Geral'],
    subscribedAt: '2025-12-10T16:45:00Z',
    opens: 15,
    clicks: 2
  },
  {
...
const MOCK_LISTS = [
  { id: 1, name: 'Geral', subscribers: 1280, description: 'Lista principal de subscritores' },
  {
    ...
    name: 'Premium',
    subscribers: 245,
    description: 'Subscritores premium com acesso antecipado'
  },
  {
    id: 3,
    name: 'Alertas',
    subscribers: 890,
    description: 'Subscritores para alertas de noticias urgentes'
  }
];
  ...
  if (!isBrevoConfigured()) {
    ...
    if (params.search) {
      ...
      filtered = filtered.filter(
        (s) => s.email.toLowerCase().includes(search) || s.name.toLowerCase().includes(search)
      );
    }
    if (params.list) {
...
export const importSubscribers = async (file) => {
  if (!isBrevoConfigured()) {
    return {
      success: true,
      imported: 0,
      message: 'Modo de demonstracao - importacao simulada',
      isMock: true
    };
  }
...
export const getCampaigns = async (params = {}) => {
  if (!isBrevoConfigured()) {
    ...
    if (params.search) {
      const search = params.search.toLowerCase();
      filtered = filtered.filter(
        (c) => c.name.toLowerCase().includes(search) || c.subject.toLowerCase().includes(search)
      );
    }
