...
const UsersPage = () => {
  ...
  const RoleBadge = ({ role }) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${styles[role] || 'bg-gray-100'}`}
      >
        {labels[role] || role}
      </span>
    );
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        ...
        <div className="md:hidden divide-y divide-gray-200">
          {users.length === 0 ? (
            <div className="p-6 text-center text-gray-500">Nenhum usuario encontrado</div>
          ) : (
            users.map((u) => (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Senha{' '}
                  {editingUser && (
                    <span className="text-gray-400 font-normal">(deixe vazio para manter)</span>
                  )}
                </label>
                <input
                  type="password"
