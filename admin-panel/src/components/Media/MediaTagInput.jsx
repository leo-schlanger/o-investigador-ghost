  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        const filtered = tags.filter((t) => !selectedTags.some((st) => st.id === t.id));
    ...
    <div ref={containerRef} className="relative">
      ...
      <div className="flex flex-wrap gap-1.5 mb-2">
        {selectedTags.map((tag) => (
          ...
          >
            {tag.name}
            <button onClick={() => handleRemoveTag(tag.id)} className="hover:text-blue-900">
              <X size={12} />
            </button>
          </span>
        ))}
      ...
      <div className="relative">
        ...
        {showSuggestions && (suggestions.length > 0 || inputValue.trim()) && (
          <div className="absolute z-10 w-full mt-1 bg-white border rounded shadow-lg max-h-48 overflow-y-auto">
            {loading && <div className="px-3 py-2 text-sm text-gray-500">A carregar...</div>}
              ...
              suggestions.map((tag, index) => (
                ...
                >
                  ...
                  {tag.usageCount !== undefined && (
                    <span className="text-xs text-gray-400">{tag.usageCount} usos</span>
                  )}
                </button>
              ))}
