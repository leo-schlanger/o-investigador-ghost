...
const MediaFilters = ({ searchTerm, onSearchChange, selectedTagIds = [], onTagFilterChange }) => {
          ...
          {showTagFilter && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1 bg-white border rounded-lg shadow-lg py-2 z-20 min-w-[200px] max-h-64 overflow-y-auto">
                {allTags.length === 0 ? (
                  <p className="px-3 py-2 text-sm text-gray-500">Nenhuma tag disponivel</p>
                ) : (
                  <>
                    {allTags.map((tag) => (
                      <label
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {selectedTagIds.map((tagId) => {
            ...
            return (
              ...
              >
                {tag.name}
                <button onClick={() => handleTagToggle(tag.id)} className="hover:text-blue-900">
                  <X size={10} />
                </button>
              </span>
            );
